...
use lib './http-message-master/lib'; #When launching on CloudBees, this will tell it where to find the HTTP libraries.
...
$runCompString = "scommand.bat cmd=play name=\"/$compName\" username=$username password=$password url=$soastaUrl wait=yes format=junitxml file=1-SOASTA_RESULTS_ID.xml $scommandoptions";
